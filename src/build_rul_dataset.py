...
sensor_readings["timestamp"] = pd.to_datetime(sensor_readings["timestamp"], errors="coerce")
robot_units["installation_date"] = pd.to_datetime(robot_units["installation_date"], errors="coerce")
failure_events["failure_time"] = pd.to_datetime(failure_events["failure_time"], errors="coerce")
...
def add_maintenance_features(rul_dataset, maintenance_logs):
    ...
    return pd.concat(
        [rul_dataset.reset_index(drop=True), pd.DataFrame(features)],
        axis=1
    )
...
for col in sensor_cols:
    rul_dataset[f"{col}_rolling_mean_24h"] = (
        rul_dataset.groupby("robot_id")[col]
        .transform(lambda x: x.rolling(window=4, min_periods=1).mean())
    )

    rul_dataset[f"{col}_rolling_std_24h"] = (
        rul_dataset.groupby("robot_id")[col]
        .transform(lambda x: x.rolling(window=4, min_periods=1).std())
    )

    rul_dataset[f"{col}_change_rate"] = (
        rul_dataset.groupby("robot_id")[col].diff()
    )
...
maintenance_logs.to_csv(PROCESSED_DIR / "maintenance_logs_with_timestamps.csv", index=False)
failure_events_augmented.to_csv(PROCESSED_DIR / "failure_events_augmented.csv", index=False)
